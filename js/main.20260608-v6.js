const MAIN_SCRIPT_VERSION = "main.20260608-v6";
...
function setupDampedScroll() {
  ...
  window.addEventListener("wheel", (event) => {
    if (event.ctrlKey || event.metaKey || event.shiftKey || document.body.classList.contains("profile-open")) return;

    event.preventDefault();
    targetY = Math.max(0, Math.min(maxScrollY(), targetY + event.deltaY));
    if (!frame) {
