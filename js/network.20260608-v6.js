const NETWORK_SCRIPT_VERSION = "network.20260608-v6";
...
if (canvas) {
  ...
  function getDDTarget(index, total, width, height, name) {
    ...
    const letterWidth = Math.min(width * 0.23, 310);
    const letterHeight = Math.min(height * 0.64, 520);
    const gap = Math.min(width * 0.08, 110);
    ...
    const verticalRatio = 0.34;
    const t = localCount <= 1 ? 0 : localIndex / (localCount - 1);
    let x;
    let y;

    if (t < verticalRatio) {
      const verticalT = t / verticalRatio;
      x = leftEdge;
      y = top + verticalT * letterHeight;
    } else {
      const arcT = (t - verticalRatio) / (1 - verticalRatio);
      x = leftEdge + Math.sin(arcT * Math.PI) * letterWidth;
      y = top + arcT * letterHeight;
    }
