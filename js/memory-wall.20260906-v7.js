const MEMORY_WALL_VERSION = "memory-wall.20260906-v7";
...
(function initMemoryWall() {
  ...
  const SELECT_SCALE_MOBILE = 2.05;
  const LERP = 0.28;
  const GAP = 6;
  const MOVE_EPS = 0.12;
  ...
  function measureColumns() {
    ...
    gap = mobile ? 3 : GAP;
    // Phone: denser 6–7 cols. Large / 4K: wider cols so fewer tiles → fewer repeats.
    const targetCol = mobile
      ? Math.max(40, Math.min(50, width / 7.5))
      : width >= 3000
        ? 100
        : width >= 2200
          ? 80
          : width >= 1500
            ...
            : width < 1100
              ? 48
              : 56;
    const minCols = mobile ? 6 : 8;
    cols = Math.max(minCols, Math.floor((width + gap) / (targetCol + gap)));
    if (mobile) cols = Math.min(Math.max(cols, 6), 7);
    colW = (width - gap * (cols - 1)) / cols;
    wall.style.width = "100%";
    // Desktop: lock to viewport. Mobile: height is set after masonry (scrollable).
    if (!mobile) {
  ...
  function buildMasonry() {
    ...
    const { width, height, mobile } = measureColumns();
    ...
    // Mobile: place the full unique set so users can scroll to see everything.
    // Desktop: pack the viewport height.
    ...
    while (placed < maxItems) {
      ...
      if (!mobile && colHeights[shortest] >= height) break;

      const imageIndex = takeImage();
      const aspect = aspectCache[imageIndex] || 1;
      const tileH = Math.max(mobile ? 36 : 28, colW * aspect);

      // Desktop: allow slight overflow so columns stay visually full (clipped).
      ...
      img.alt = "";
      img.loading = placed < 36 ? "eager" : "lazy";
      img.decoding = "async";
      img.draggable = false;
    ...
    if (mobile) {
      const contentH = Math.max(height, ...colHeights);
      wall.style.height = `${Math.ceil(contentH)}px`;
      document.body.classList.add("memory-scrollable");
    } else {
      document.body.classList.remove("memory-scrollable");
    }
  ...
  function flushPointer() {
    ...
    if (!event) return;
    // Recalc on mobile — wall moves under the finger when the page scrolls.
    if (!wallRect || isMobileWidth()) wallRect = wall.getBoundingClientRect();
  ...
  function syncPointer(event) {
    if (isMobileWidth()) {
      ...
      if (event.type === "pointermove" && pointerDownAt) {
        const dx = event.clientX - pointerDownAt.x;
        const dy = event.clientY - pointerDownAt.y;
        // Finger slid → treat as page scroll, drop selection so scroll stays smooth.
  ...
  wall.addEventListener("pointerup", onPointerUp, { passive: true });
