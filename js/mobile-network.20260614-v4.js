(() => {
const MOBILE_NETWORK_SCRIPT_VERSION = "mobile-network.20260614-v4";
console.info(`[site] ${MOBILE_NETWORK_SCRIPT_VERSION}`);
...
if (canvas) {
  ...
  function getMorphAmount(time) {
    const phase = (time % 20000) / 20000;
    if (phase < 0.38) return 0;
    if (phase < 0.6) return easeInOut((phase - 0.38) / 0.22);
    if (phase < 0.78) return 1;
    return 1 - easeInOut((phase - 0.78) / 0.22);
  }
  ...
  function getDDTarget(index, total, width, height, name) {
    ...
    const maxTotalWidth = width * (isMobile ? 0.76 : 0.65);
    const gap = Math.min(width * (isMobile ? 0.055 : 0.07), isMobile ? 34 : 118);
    const letterWidth = Math.min((maxTotalWidth - gap) / 2, isMobile ? 168 : 440);
    const targetRatio = isMobile ? 1.55 : 1.42;
    const letterHeight = Math.min(letterWidth * targetRatio, height * (isMobile ? 0.46 : 0.7), isMobile ? 260 : 610);
    const totalWidth = letterWidth * 2 + gap;
    const leftEdge = width / 2 - totalWidth / 2 + letterIndex * (letterWidth + gap);
    const headerSafeTop = isMobile ? 76 : 78;
    const top = Math.max(headerSafeTop + 22, height / 2 - letterHeight / 2 + (isMobile ? 2 : 12));
    const segments = [
    ...
    groups.forEach((group, groupIndex) => {
      ...
      for (let i = 0; i < ordered.length; i += 1) {
        ...
        [b, c].forEach((target, targetIndex) => {
          ...
          ctx.strokeStyle = `rgba(232, 240, 255, ${(0.13 - targetIndex * 0.04) * alphaScale})`;
          ctx.lineWidth = targetIndex === 0 ? 1.05 : 0.72;
          ctx.stroke();
        });
      }
      ...
      group.forEach((node, index) => {
        ...
        offsets.forEach((offset, offsetIndex) => {
          ...
          const alpha = (0.026 + offsetIndex * 0.003 + distanceFade * 0.07) * alphaScale;
  ...
  function getLabelSize(isActive, morphAmount = 0) {
    ...
    if (viewportWidth <= 420) return isActive ? 11.6 : Math.max(7.8, 8.5 - morphAmount * 0.2);
    if (viewportWidth <= 680) return isActive ? 12.2 : Math.max(8.4, 9.2 - morphAmount * 0.28);
    return isActive ? 13 : 10.8 - morphAmount * 0.55;
  }

  function updateLabelMetrics(node, width, isActive, morphAmount = 0) {
    const headerSafeTop = window.innerWidth <= 680 ? 72 : 78;
  ...
  function draw(time) {
    ...
    nodes.forEach((node) => {
      ...
      ctx.font = `${getLabelSize(isActive, morphAmount)}px Inter, system-ui, sans-serif`;
      ctx.fillStyle = isActive ? purple.text : `rgba(246,244,237,${0.82 + morphAmount * 0.12})`;
