(() => {
const MOBILE_NETWORK_SCRIPT_VERSION = "mobile-network.20260614-v6";
...
if (canvas) {
  ...
  function getDDTarget(index, total, width, height, name) {
    ...
    const bottomSafe = isMobile ? 86 : 30;
    const letterGap = Math.min(width * (isMobile ? 0.08 : 0.07), isMobile ? 34 : 118);
    const targetRatio = isMobile ? 1.45 : 1.42;
    let letterWidth;
    let letterHeight;
    let leftEdge;
    let top;
    ...
    if (isMobile) {
      ...
      letterWidth = Math.min(width * 0.48, 190);
      letterHeight = Math.min(letterWidth * targetRatio, (availableHeight - letterGap) / 2, 248);
