(() => {
const MOBILE_NETWORK_SCRIPT_VERSION = "mobile-network.20260614-v14";
...
if (canvas) {
  ...
  function drawGlobalConnectionField(width, height, morphAmount) {
    const isPhone = isPhoneLayout(width);
    const baseAlpha = isPhone ? 0.006 : 0.028;
    const morphBoost = 1 + morphAmount * 0.55;

    ctx.save();
    ctx.lineWidth = isPhone ? 0.16 : 0.34;
    for (let i = 0; i < nodes.length; i += 1) {
      for (let j = i + 1; j < nodes.length; j += 1) {
        const a = nodes[i];
        const b = nodes[j];
        const alpha = baseAlpha * morphBoost;
  ...
  function draw(time) {
    ...
    if (!isPhoneLayout(width) || morphAmount > 0.18) {
