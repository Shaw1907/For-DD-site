const NETWORK_SCRIPT_VERSION = "network.20260614-v29";
...
const profileHeroWrap = document.querySelector("#profileHeroWrap");
const profileWorkHero = document.querySelector("#profileWorkHero");
...
if (canvas) {
  ...
  function setProfileImage(wrap, image, src, alt) {
  ...
  function openProfilePanel(node) {
    ...
    if (profile) {
      ...
      if (youtube) appendContactRow("youtube", youtube.replace(/^https?:\/\//, ""), youtube);
      if (profile.rednote) {
        const rednoteHref = profile.rednoteUrl
          || `https://www.xiaohongshu.com/search_result?keyword=${encodeURIComponent(profile.rednote)}`;
    ...
    setProfileImage(profilePortraitWrap, profilePortrait, images.portrait || "", imageAlt);
    setProfileImage(profileHeroWrap, profileWorkHero, images.hero || "", imageAlt);
