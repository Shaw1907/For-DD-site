(() => {
const MOBILE_NETWORK_SCRIPT_VERSION = "mobile-network.20260614-v1";
...
if (canvas) {
  ...
  function updateLabelMetrics(node, width, isActive, morphAmount = 0) {
    ...
    const mobileLabelBoost = window.innerWidth <= 680 ? 0.8 : 0;
    const labelSize = (isActive ? 13 : 10.8 - morphAmount * 0.55) + mobileLabelBoost;
  ...
  function draw(time) {
    ...
    nodes.forEach((node) => {
      ...
      if (width > 700 || isActive) {
        ctx.font = `${isActive ? 13 : 10.8 - morphAmount * 0.55}px Inter, system-ui, sans-serif`;
