(() => {
const MOBILE_NETWORK_SCRIPT_VERSION = "mobile-network.20260614-v5";
...
if (canvas) {
  ...
  function placeReadableMobileLabel(node, occupiedRects, width, height, labelSize, isActive) {
  ...
  function draw(time) {
    ...
    labelNodes.forEach((node) => {
      ...
      if (!placeReadableMobileLabel(node, occupiedLabelRects, width, height, labelSize, isActive)) return;

      ctx.font = `${labelSize}px Inter, system-ui, sans-serif`;
      ctx.lineWidth = window.innerWidth <= 680 ? 3.8 : 0;
      ctx.strokeStyle = "rgba(0,0,0,0.82)";
      if (window.innerWidth <= 680) ctx.strokeText(node.name, node.labelX, node.labelY);
