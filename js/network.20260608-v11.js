const NETWORK_SCRIPT_VERSION = "network.20260608-v11";
...
if (canvas) {
    ...
    groups.forEach((group, groupIndex) => {
      group.forEach((node, index) => {
        ...
        offsets.forEach((offset, offsetIndex) => {
          ...
          const distanceFade = Math.max(0.18, 1 - distance / Math.max(canvas.clientWidth * 0.36, 240));
          const alpha = (0.018 + offsetIndex * 0.004 + distanceFade * 0.055) * alphaScale;
  ...
  function draw(time) {
    ...
    nodes.forEach((node) => {
      ...
      ctx.beginPath();
      ctx.arc(node.x, node.y, isActive ? node.radius + 3.8 : node.radius, 0, Math.PI * 2);
      ctx.fillStyle = isActive ? purple.node : "rgba(231,240,255,0.92)";
      ctx.shadowColor = isActive ? "rgba(190, 74, 255, 0.72)" : "rgba(231,240,255,0.8)";
      ctx.shadowBlur = isActive ? 15 : 12;
      ctx.fill();
      ctx.shadowBlur = 0;
