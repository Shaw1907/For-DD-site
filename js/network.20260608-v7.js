const NETWORK_SCRIPT_VERSION = "network.20260608-v7";
...
if (canvas) {
  ...
  function getDDTarget(index, total, width, height, name) {
    ...
    const letterWidth = Math.min(width * 0.27, 390);
    const letterHeight = Math.min(height * 0.6, 500);
    const strokeWidth = Math.min(letterWidth * 0.32, 112);
    const gap = Math.min(width * 0.055, 88);
    const totalWidth = letterWidth * 2 + gap;
    const leftEdge = width / 2 - totalWidth / 2 + letterIndex * (letterWidth + gap);
    const top = height / 2 - letterHeight / 2;
    const centerY = height / 2;
    const laneCount = 6;
    const lane = localIndex % laneCount;
    const laneStep = Math.max(1, Math.ceil(localCount / laneCount) - 1);
    const t = Math.floor(localIndex / laneCount) / laneStep;
    let x;
    let y;

    if (lane < 2) {
      const laneX = lane === 0 ? 0.22 : 0.72;
      x = leftEdge + laneX * strokeWidth;
      y = top + t * letterHeight;
    } else {
      const arcLane = lane - 2;
      const thickness = arcLane / 3;
      const angle = -Math.PI / 2 + t * Math.PI;
      const radiusX = letterWidth - strokeWidth * (0.38 + thickness * 0.58);
      const radiusY = letterHeight / 2 - strokeWidth * (0.08 + thickness * 0.32);
      x = leftEdge + strokeWidth * 0.55 + Math.cos(angle) * radiusX;
