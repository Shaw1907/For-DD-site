const MAIN_SCRIPT_VERSION = "main.20260608-v2";
...
function renderEventBanner(activeIndex = 0) {
  const tabs = document.querySelector("#eventTabs");
  const display = document.querySelector("#eventDisplay");
  if (!tabs || !display) return;

  const active = eventBanners[activeIndex];
  tabs.innerHTML = eventBanners.map((event, index) => `
    <button
      class="event-tab${index === activeIndex ? " is-active" : ""}"
      type="button"
  ...
  display.style.setProperty("--event-bg", active.bg);
  display.style.setProperty("--event-a", active.accent);
  display.innerHTML = `
    <div class="event-display-inner">
      <p class="event-venue">${active.venue}</p>
      <h3>${active.title}</h3>
  ...
  tabs.querySelectorAll("[data-event-index]").forEach((button) => {
