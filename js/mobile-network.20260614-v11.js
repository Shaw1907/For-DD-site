(() => {
const MOBILE_NETWORK_SCRIPT_VERSION = "mobile-network.20260614-v11";
...
if (canvas) {
  ...
  function openProfilePanel(node) {
    ...
    profileSummary.textContent = "Bioxxxx";
    profileMedia.textContent = node.slug;
    if (profileProjectTitle) profileProjectTitle.textContent = "Project name";
    if (profileProjectDescription) profileProjectDescription.textContent = "Project description";
  ...
  function drawGlobalConnectionField(width, height, morphAmount) {
    const maxDistance = Math.hypot(width, height);
    const isPhone = isPhoneLayout(width);
    const baseAlpha = isPhone ? 0.012 : 0.018;
    const morphBoost = 1 + morphAmount * 1.4;

    ctx.save();
    ctx.lineWidth = isPhone ? 0.42 : 0.52;
    for (let i = 0; i < nodes.length; i += 1) {
      for (let j = i + 1; j < nodes.length; j += 1) {
        const a = nodes[i];
        const b = nodes[j];
        const distance = Math.hypot(a.x - b.x, a.y - b.y);
        const distanceFade = Math.pow(Math.max(0.08, 1 - distance / maxDistance), 1.35);
  ...
  function draw(time) {
    ...
    drawGlobalConnectionField(width, height, morphAmount);
    ...
    for (let i = 0; i < nodes.length; i += 1) {
      for (let j = i + 1; j < nodes.length; j += 1) {
        ...
        if (distance < threshold) {
          drawLine(a, b, 0.32 * (1 - distance / threshold));
        }
      }
    }
