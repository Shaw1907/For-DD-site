(() => {
const MOBILE_NETWORK_SCRIPT_VERSION = "mobile-network.20260614-v15";
...
if (canvas) {
  ...
  function drawNeighborConnectionField(width, height, morphAmount) {
    const isPhone = isPhoneLayout(width);
    const neighborCount = isPhone ? 10 : 12;
    const maxFadeDistance = Math.max(Math.min(width, height) * (isPhone ? 0.72 : 0.42), 240);
    const baseAlpha = isPhone ? 0.06 : 0.08;
    const morphBoost = 1 + morphAmount * 0.28;
    const drawn = new Set();

    ctx.save();
    ctx.lineWidth = isPhone ? 0.44 : 0.62;
    for (let i = 0; i < nodes.length; i += 1) {
      const a = nodes[i];
      const nearest = nodes
        .map((node, index) => ({
      ...
      nearest.forEach(({ node: b, index: j, distance }) => {
        const key = i < j ? `${i}:${j}` : `${j}:${i}`;
        if (drawn.has(key)) return;
        drawn.add(key);

        const distanceFade = Math.max(0.24, 1 - distance / maxFadeDistance);
