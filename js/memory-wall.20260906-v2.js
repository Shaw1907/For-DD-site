const MEMORY_WALL_VERSION = "memory-wall.20260906-v2";
...
(function initMemoryWall() {
  ...
  // CSS transform scale stays on the compositor (≈60fps). JPEGs are ~640px, so
  // mobile enlarge to ~1.75× of a ~70px tile still samples under source resolution.
  const SELECT_SCALE = 2.6;
  const SELECT_SCALE_MOBILE = 1.75;
  const LERP = 0.28;
  const GAP = 6;
  const MOVE_EPS = 0.12;
  ...
  function tick(now) {
    ...
    for (let i = 0; i < tiles.length; i += 1) {
      ...
      // Transform-only: compositor path, no layout thrash.
      tile.el.style.transform =
        `translate3d(${tile.x.toFixed(2)}px, ${tile.y.toFixed(2)}px, 0) scale(${tile.scale.toFixed(4)})`;
    }
