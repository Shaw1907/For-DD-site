const MEMORY_WALL_VERSION = "memory-wall.20260906-v6";
...
(function initMemoryWall() {
  ...
  function buildMasonry() {
    ...
    // Fill viewport height adaptively. Safety cap only — do not stop early with black gaps.
    const maxItems = cols * (isMobileWidth(width) ? 32 : 56);
    let placed = 0;
    ...
    while (placed < maxItems) {
      ...
      const tileH = Math.max(isMobileWidth(width) ? 36 : 28, colW * aspect);

      // Place even if the last tile overflows slightly — wall clips with overflow:hidden
      // so columns stay visually full instead of leaving a black band.
